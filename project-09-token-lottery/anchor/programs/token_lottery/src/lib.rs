...
pub mod token_lottery {
    ...
    pub fn claim_prize(ctx: Context<ClaimPrize>) -> Result<()> {
        ...
        // ** is used to dereference a mutable reference to the lamports of an account.
        **ctx.accounts.token_lottery.to_account_info().lamports.borrow_mut() -= ctx.accounts.token_lottery.lottery_pot_amount;
        **ctx.accounts.payer.lamports.borrow_mut() += ctx.accounts.token_lottery.lottery_pot_amount;
...
pub struct BuyTicket<'info> {
    ...
    pub destination: InterfaceAccount<'info, TokenAccount>, // ticket_account
