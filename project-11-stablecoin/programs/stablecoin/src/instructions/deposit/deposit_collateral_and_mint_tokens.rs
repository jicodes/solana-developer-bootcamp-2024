use crate::{
  ...
  SEED_CONFIG_ACCOUNT, SEED_COLLATERAL_ACCOUNT, SEED_SOL_ACCOUNT,
};
...
pub struct DepositCollateralAndMintTokens<'info> {
  ...
  #[account(
      ...
      has_one = mint_account, // constraint: check the config account has correct mint account
  )]
  pub config_account: Box<Account<'info, Config>>, // use Box for program size optimization
  #[account(
      init_if_needed,
      payer = depositor,
  ...
  pub sol_account: SystemAccount<'info>, // holding lamports 
  #[account(mut)]
...
pub fn process_deposit_collateral_and_mint_tokens(
  ...
  // initialize collateral account if needed
